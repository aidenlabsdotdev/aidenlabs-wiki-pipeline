...
- **LLM phases use `delegate_task`** — avoids creating agent sessions in state.db
- **`_meta/` is read input → written output** — taxonomy.md stays manual; everything else regenerated by `update-meta`
- **Idempotent** — journal skipped if exists, projects additive, synthesis skips existing pairs
